...
class YBY_Connector {
	...
	private static function reconcile_payout( $input, $payout_id ) {
		...
		$ids = self::payout_referral_ids( $payout );
		...
		$payout = self::provider_payout( $payout_id ); $final_ids = self::payout_referral_ids( $payout ); if ( $final_ids !== $input['referral_ids'] ) { return self::provider_failure( 'PROVIDER_SYNC_FAILED', 'Payout referral read-back needs retry.' ); }
	...
	private static function payout_item( $payout ) {
		$referral_ids = self::payout_referral_ids( $payout );
