...
function createRuntime(authResult) {
  const events = [];
  const requests = [];
  ...
  const window = {
    ...
    fetch(url, options) {
      ...
      if (url.includes("/challenge") && requests.length === 1) {
        data = { success: true, nonce: "fresh-one-tap-challenge" };
      } else if (url.endsWith("/onetap")) {
        data = authResult;
      } else {
        data = { success: false, code: "already_authenticated", message: "You are already signed in." };
      }
