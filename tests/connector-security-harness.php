...
YBY_Connector::register_routes(); security_assert( 5 === count( $routes ) && isset( $routes['andy-core/v1/erp/health'] ) && 'GET' === $routes['andy-core/v1/erp/health']['methods'], 'M3 must retain the secured health route and register exactly four read-only snapshots.' );
$endpoint_statuses = YBY_Connector::endpoint_statuses(); security_assert( 10 === count( $endpoint_statuses ) && true === $endpoint_statuses['health']['available'], 'Configured HTTPS health canary must be available.' ); foreach ( array( 'affiliate_snapshot','coupon_snapshot','referral_snapshot','payout_snapshot' ) as $name ) { security_assert( false === $endpoint_statuses[$name]['available'] && 'Provider Missing' === $endpoint_statuses[$name]['status'], 'Implemented snapshots must fail closed when providers are absent.' ); } foreach ( array( 'affiliate_provision','affiliate_status','coupon_check','coupon_provision','payout_complete' ) as $name ) { security_assert( false === $endpoint_statuses[$name]['available'] && 'Not Available' === $endpoint_statuses[$name]['status'], 'M3 POST routes must remain unavailable.' ); }
