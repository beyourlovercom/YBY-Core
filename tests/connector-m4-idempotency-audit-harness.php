...
$content = YBY_Connector_Idempotency::begin( 'site-a', 'content.publish', 'content-layout-77', array( 'erp_article_id' => 77, 'layout_snapshot_id' => 9, 'preview_hash' => str_repeat( 'a', 64 ) ) );
m4_assert( 'acquired' === $content['status'], 'Content publish must acquire a durable mutation identity.' );
m4_assert( true === YBY_Connector_Idempotency::succeed( $content['id'], array( 'resource_contract_version' => 'content-publish-v1', 'provider' => 'wordpress', 'post_id' => 901, 'external_id' => '901', 'status' => 'draft', 'url' => 'https://example.test/blog/test/', 'created' => true, 'layout_snapshot_id' => 9, 'preview_hash' => str_repeat( 'a', 64 ), 'canonical_path_expected' => '/blog/test/', 'canonical_path_match' => true, 'indexing' => 'noindex' ) ), 'Content publish result must persist in the safe replay envelope.' );
$content_replay = YBY_Connector_Idempotency::begin( 'site-a', 'content.publish', 'content-layout-77', array( 'layout_snapshot_id' => 9, 'preview_hash' => str_repeat( 'a', 64 ), 'erp_article_id' => 77 ) );
m4_assert( 'replay' === $content_replay['status'] && 901 === $content_replay['record']['result']['post_id'] && 'https://example.test/blog/test/' === $content_replay['record']['result']['url'] && 'noindex' === $content_replay['record']['result']['indexing'], 'Content publish replay must preserve post identity and URL.' );
