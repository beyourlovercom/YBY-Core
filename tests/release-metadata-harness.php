<?php
/** Release metadata contract for Andy Core v1.5.6. */
...
$builder     = $read( 'scripts/build-v1.5.6-release.sh' );
...
$assert( false !== strpos( $plugin, 'Version:           1.5.6' ), 'PLUGIN_HEADER_VERSION_GATE failed.' );
$assert( false !== strpos( $plugin, "define( 'YBY_CORE_VERSION', '1.5.6' );" ), 'CORE_CONSTANT_VERSION_GATE failed.' );
$assert( false !== strpos( $plugin, "define( 'YBY_DATABASE_VERSION', '1.4.0' );" ), 'DATABASE_VERSION_GATE failed.' );
$assert( false !== strpos( $version, 'Stable Version: 1.5.6' ) && false !== strpos( $version, 'Database Version: 1.4.0' ), 'VERSION_MD_GATE failed.' );
$assert( false !== strpos( $readme, 'Product version: `1.5.6`' ) && false !== strpos( $readme, 'Database version: `1.4.0`' ), 'README_VERSION_GATE failed.' );
$assert( false !== strpos( $wp_readme, 'Stable tag: 1.5.6' ) && false !== strpos( $wp_readme, 'Tested up to: 7.1' ), 'README_METADATA_GATE failed.' );
$assert( false !== strpos( $changelog, '## v1.5.6 - 2026-09-07' ), 'CHANGELOG_RELEASE_GATE failed.' );

foreach ( array( 'RELEASE_NOTES_v1.5.6.md', 'UPGRADE_GUIDE_v1.5.6.md', 'ROLLBACK_GUIDE_v1.5.6.md', 'docs/v1.5.6/ANDY-CORE-PUBLIC-RELEASE-CHANNEL-M2.md' ) as $file ) {
	$assert( is_file( $root . '/' . $file ), 'Release/WP documentation missing: ' . $file );
}
foreach ( array( 'RELEASE_NOTES_v1.5.5.md', 'UPGRADE_GUIDE_v1.5.5.md', 'ROLLBACK_GUIDE_v1.5.5.md', 'scripts/build-v1.5.5-release.sh', 'docs/v1.5.5/ANDY-CORE-v1.5.5-FIRST-NATIVE-UPGRADE-UAT.md', 'RELEASE_NOTES_v1.5.4.md', 'UPGRADE_GUIDE_v1.5.4.md', 'ROLLBACK_GUIDE_v1.5.4.md', 'scripts/build-v1.5.4-release.sh', 'docs/v1.5.4/ANDY-CORE-SECURE-UPDATER-M1.md', 'docs/v1.5.4/ANDY-CORE-RELEASE-AUTOMATION-M1.md', 'RELEASE_NOTES_v1.5.3.md', 'UPGRADE_GUIDE_v1.5.3.md', 'ROLLBACK_GUIDE_v1.5.3.md', 'scripts/build-v1.5.3-release.sh' ) as $file ) {
...
$assert( false !== strpos( $validation, 'php tests/secure-updater-harness.php' ) && false !== strpos( $validation, 'andy-core-v1.5.6-release-candidate' ), 'PR_RELEASE_VALIDATION_GATE failed.' );
