<?php
/** v1.7.0 release-candidate metadata + backward-compatible signed updater contract. */
...
$builder     = $read( 'scripts/build-v1.7.0-release.sh' );
...
$assert( false !== strpos( $plugin, 'Version:           1.7.0' ), 'PLUGIN_HEADER_VERSION_GATE failed.' );
$assert( false !== strpos( $plugin, "define( 'YBY_CORE_VERSION', '1.7.0' );" ), 'CORE_CONSTANT_VERSION_GATE failed.' );
...
$assert( false !== strpos( $validation, 'php tests/secure-updater-harness.php' ) && false !== strpos( $validation, 'updater-backward-compat-harness.php' ) && false !== strpos( $validation, 'andy-core-v1.7.0-release-candidate' ), 'PR_RELEASE_VALIDATION_GATE failed.' );
