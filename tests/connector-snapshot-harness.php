...
snapshot_assert( 16 === count( $routes ) && isset( $routes[ YBY_Connector::REST_NAMESPACE . '/affiliates/check' ] ) && isset( $routes[ YBY_Connector::REST_NAMESPACE . '/wordpress/users/(?P<user_id>\\d+)/password' ] ) && isset( $routes[ YBY_Connector::REST_NAMESPACE . '/content/preview' ] ) && isset( $routes[ YBY_Connector::REST_NAMESPACE . '/content/publish' ] ), 'Connector must register seven GET and nine POST routes.' );
...
snapshot_assert( 15 === count( $statuses ) && 'GET' === $statuses['subscriber_snapshot']['method'] && isset( $statuses['content_preview'], $statuses['content_publish'] ), 'Connector must expose exactly fifteen contract rows including Content preview/publish.' );
...
snapshot_assert( 9 === substr_count( $route_block, "'POST'" ), 'Exactly nine POST routes must be registered.' );
