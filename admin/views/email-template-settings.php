...
<div class="yby-email-os">
	...
	<?php if ( YBY_Email_Template_Admin::SECTION_OVERVIEW === $section ) : ?>
		...
		<div class="yby-email-os__panel">
			<div class="yby-email-os__panel-head">
				...
				<span><?php esc_html_e( '点击模板名称直接进入当前编辑器；备注为 Owner Metadata，不参与 Runtime。', 'yby-core' ); ?></span>
			</div>
			<?php if ( ! empty( $registry_notice ) ) : ?><div class="notice notice-<?php echo 'success' === ( $registry_notice['type'] ?? '' ) ? 'success' : 'error'; ?> inline"><p><?php echo esc_html( $registry_notice['message'] ?? '' ); ?></p></div><?php endif; ?>
			<table class="widefat striped yby-email-os__table">
				<thead><tr><th><?php esc_html_e( '模板', 'yby-core' ); ?></th><th><?php esc_html_e( '来源', 'yby-core' ); ?></th><th><?php esc_html_e( '备注', 'yby-core' ); ?></th><th><?php esc_html_e( '受众', 'yby-core' ); ?></th><th><?php esc_html_e( 'Runtime', 'yby-core' ); ?></th><th><?php esc_html_e( '管理模式', 'yby-core' ); ?></th></tr></thead>
				<tbody>
				<?php foreach ( $templates as $template ) : ?>
					<?php $registry_key = (string) ( $template['template_key'] ?? '' ); $template_edit_url = $this->template_editor_url( $template, $base_url ); if ( 'woocommerce' === ( $template['provider'] ?? '' ) && ! empty( $legacy_governance['mappings'][ $template['source_id'] ]['editor_url'] ) ) { $template_edit_url = $legacy_governance['mappings'][ $template['source_id'] ]['editor_url']; } $registry_note = (string) ( $owner_notes[ $registry_key ] ?? '' ); ?>
					<tr>
						<td><?php if ( $template_edit_url ) : ?><a class="yby-email-os__template-link" href="<?php echo esc_url( $template_edit_url ); ?>"><strong><?php echo esc_html( $template['label'] ); ?></strong><span aria-hidden="true">↗</span></a><?php else : ?><strong><?php echo esc_html( $template['label'] ); ?></strong><?php endif; ?><br><code><?php echo esc_html( $registry_key ); ?></code></td>
						<td><?php echo esc_html( $template['provider'] ); ?></td>
						<td class="yby-email-os__note-cell"><form method="post" class="yby-email-os__note-form"><?php wp_nonce_field( 'yby_email_registry_note', 'yby_email_registry_note_nonce' ); ?><input type="hidden" name="registry_template_key" value="<?php echo esc_attr( $registry_key ); ?>"><input type="text" name="registry_note" maxlength="120" value="<?php echo esc_attr( $registry_note ); ?>" placeholder="<?php esc_attr_e( '输入简短用途说明', 'yby-core' ); ?>"><button type="submit" class="button button-small" name="yby_email_registry_note_action" value="save"><?php esc_html_e( '保存', 'yby-core' ); ?></button></form></td>
	...
	<?php elseif ( YBY_Email_Template_Admin::SECTION_WOOCOMMERCE === $section ) : ?>
		...
		<div class="yby-email-os__panel yby-email-os__governance"><strong><?php esc_html_e( 'Legacy Customizer Governance', 'yby-core' ); ?></strong><span><?php esc_html_e( 'VillaTheme 保持激活并继续负责 Woo Runtime；Email OS 只读扫描模板映射、Rule Variants、默认模板与异常绑定，不自动修改任何 Legacy 数据。', 'yby-core' ); ?></span></div>
		<div class="yby-email-os__stats">
		...
		<div class="yby-email-os__stats">
			<div class="yby-email-os__stat"><strong><?php echo esc_html( $legacy_governance['published'] ?? 0 ); ?></strong><span><?php esc_html_e( 'Legacy Published', 'yby-core' ); ?></span></div>
			<div class="yby-email-os__stat"><strong><?php echo esc_html( $legacy_governance['mapped_woo'] ?? 0 ); ?></strong><span><?php esc_html_e( '已映射 Woo 类型', 'yby-core' ); ?></span></div>
			<div class="yby-email-os__stat"><strong><?php echo esc_html( $legacy_governance['rule_variant_types'] ?? 0 ); ?></strong><span><?php esc_html_e( 'Rule Variant 类型', 'yby-core' ); ?></span></div>
			<div class="yby-email-os__stat"><strong><?php echo esc_html( count( (array) ( $legacy_governance['warnings'] ?? array() ) ) ); ?></strong><span><?php esc_html_e( '治理提醒', 'yby-core' ); ?></span></div>
		</div>
		<?php include YBY_CORE_PLUGIN_DIR . 'admin/views/email-template-legacy-governance.php'; ?>
		<div class="yby-email-os__panel">
			...
			<table class="widefat striped yby-email-os__table">
				<thead><tr><th><?php esc_html_e( '模板', 'yby-core' ); ?></th><th><?php esc_html_e( '收件人', 'yby-core' ); ?></th><th><?php esc_html_e( '当前编辑器', 'yby-core' ); ?></th><th><?php esc_html_e( '治理', 'yby-core' ); ?></th><th>Runtime</th><th><?php esc_html_e( '状态', 'yby-core' ); ?></th><th><?php esc_html_e( '操作', 'yby-core' ); ?></th></tr></thead>
				<tbody>
				<?php foreach ( $woo_templates as $template ) : ?>
					<?php $woo_map = (array) ( $legacy_governance['mappings'][ $template['source_id'] ] ?? array() ); $governed_editor_url = ! empty( $woo_map['editor_url'] ) ? $woo_map['editor_url'] : $template['editor_url']; ?>
					<tr>
						<td><a class="yby-email-os__template-link" href="<?php echo esc_url( $governed_editor_url ); ?>"><strong><?php echo esc_html( $template['label'] ); ?></strong><span aria-hidden="true">↗</span></a><br><code><?php echo esc_html( $template['source_id'] ); ?></code></td>
						...
						<td><?php $governance_state = (string) ( $woo_map['state'] ?? 'woocommerce_native' ); if ( 'rule_variants' === $governance_state ) : ?><span class="yby-email-os__status is-bridge">Rule Variants ×<?php echo absint( $woo_map['published_count'] ?? 0 ); ?></span><?php elseif ( 'review' === $governance_state ) : ?><span class="yby-email-os__status is-off">需检查 ×<?php echo absint( $woo_map['published_count'] ?? 0 ); ?></span><?php elseif ( 'mapped' === $governance_state ) : ?><span class="yby-email-os__status is-ready">单模板</span><?php else : ?><span class="yby-email-os__status">Woo 原生</span><?php endif; ?></td>
						...
						<td class="yby-email-os__actions"><a class="button button-small" href="<?php echo esc_url( $template['settings_url'] ); ?>"><?php esc_html_e( 'Woo 设置', 'yby-core' ); ?></a><?php if ( 'villatheme' === ( $template['current_editor'] ?? '' ) ) : ?><a class="button button-small" href="<?php echo esc_url( $governed_editor_url ); ?>"><?php esc_html_e( 'Customizer 编辑', 'yby-core' ); ?></a><?php endif; ?><a class="button button-small" href="<?php echo esc_url( $governed_editor_url ); ?>"><?php esc_html_e( '预览 / 测试', 'yby-core' ); ?></a></td>
					</tr>
				<?php endforeach; ?>
				</tbody>
			</table>
...
\t<?php elseif ( in_array( $section, array( YBY_Email_Template_Admin::SECTION_WORDPRESS, YBY_Email_Template_Admin::SECTION_ANDY_CORE ), true ) ) : ?>
		...
		<div class="yby-email-os__native-layout">
			<aside class="yby-email-os__panel yby-email-os__native-list">
				...
				<p><?php esc_html_e( '仅管理 Email OS 原生模板；不会改变 WooCommerce Runtime。Published 版本是 WordPress / Andy Core Native Runtime 的唯一来源。', 'yby-core' ); ?></p>
