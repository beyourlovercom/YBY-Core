...
## KOL create auto-provision freeze evidence (2026-09-04)

- Added the authenticated, read-only `POST /affiliates/check` contract. Its body is exactly `{email}`. The response contains availability and, when found, only `wp_user_id` and/or `affiliate_id`; the email is never echoed.
- Extended `POST /affiliates/provision` without changing the legacy contract. The exact six-field body remains the manual-provision path with random password generation and existing-user/Affiliate reuse. The exact strict create body adds `password` and `existing_account_policy: "reject"`; supplied passwords are accepted only for new subscriber users and must be 12–128 characters with at least one ASCII letter and one digit.
- Strict create rejects an existing WordPress email/user or Affiliate with HTTP 409 unless the same connection/KOL already has a ready Connector binding or the request is an exact durable idempotency replay. The existing per-KOL lease, unique binding guards, provider re-checks, and insert conflict handling remain the race-safety authority; ambiguous state fails closed.
- Plaintext passwords are not logged, audited, returned, or included in `safe_result`. Idempotency persistence contains only the one-way request fingerprint; audit persistence retains only the existing allowlisted provider IDs and safe mutation metadata.
- Regression coverage in `tests/connector-m5-affiliate-harness.php` covers available check/no email echo, existing WordPress/Affiliate conflicts, supplied-password creation, invalid password rejection, strict race protection, exact idempotent replay, and serialized idempotency/audit absence of plaintext password evidence. No password value is documented here.
- This worktree-only increment includes no UI/menu changes, commit, push, Dev/WWW deployment, or production call.
